...
def plot_OLR(datetime,dmet,figax=None, scale=1, lonlat=None, steps=[0,2], coast_details="auto", model=None, domain_name=None,
  ...
  for leadtime in np.array(steps): #
      ...
      data[mask] = np.nan
      sc=ax1.pcolormesh(x, y, data[ :, :], vmin=-230,vmax=-110, cmap=plt.cm.Greys_r)
      ...
      #cb=fig1.colorbar(sc, label='OLR / W m^{-2}',cax = cax, orientation = 'horizontal')
      ax_cb = adjustable_colorbar_cax(fig1, ax1)
      cb = plt.colorbar(sc, fraction=0.046, pad=0.01, ax=ax1, aspect=25, cax= ax_cb, label='OLR / W m^{-2}',
                                      extend="both")
      #cb=fig1.colorbar(sc, label='OLR / W m^{-2}',orientation = 'horizontal')
      cb.ax.tick_params(labelsize=20) 
      #cb = colorbar(label='a label')
      #ax = cb.ax
      #text = ax.xaxis.label
      #font = matplotlib.font_manager.FontProperties(size=20)#family='times new roman', style='italic', 
