...
class PMET():
    ...
    def retrieve_handler(self):
        ...
        dmet, data_domain, bad_param = checkget_data_handler(all_param=self.param, date=self.date, model=self.model,
                                                             step=self.steps,
                                                             p_level=self.p_level, m_level=self.m_level, mbrs=self.mbrs,
                                                             domain_name=self.domain_name,
                                                             domain_lonlat=self.domain_lonlat, point_name=self.point_name[0])
    ...
    def calculations(self):
        ...
        self.dmet.ug_10m, self.dmet.vg_10m = xwind2uwind(self.dmet.x_wind_gust_10m, self.dmet.y_wind_gust_10m,
                                                         self.dmet.alpha)
    ...
    def points(self):
        ...
        if point_lonlat:
            ...
            print(package_path)
            op = package_path + "../data/sites.csv"
            print(op)
            sites = pd.read_csv(op, sep=";", header=0, index_col=0)
            print(sites)
            print(sites.loc[self.point_name].lon.item())
            #exit(1)
            ind_list = nearest_neighbour(sites.loc[self.point_name].lon.item(), sites.loc[self.point_name].lat.item(), dmet.longitude,
        ...
        #print(self.dmet.land_area_fraction)
        #print(np.shape(self.dmet.land_area_fraction ))

        landfaction1time=self.dmet.land_area_fraction[0, :, :] if len(np.shape(self.dmet.land_area_fraction )) > 3 else self.dmet.land_area_fraction[0]
        sst1time = self.dmet.SST[0, :, :].mask if len(np.shape(self.dmet.SST )) > 3 else np.ma.array([self.dmet.SST[0]]).mask
        #print(self.dmet.SST[0])
        #print(type(self.dmet.SST))
        #print(self.dmet.SST.mask)
        indx_sea = np.where((landfaction1time == 0) & (~sst1time))
        indx_land = np.where((landfaction1time == 1) & (sst1time))
    ...
    def plot_meteogram(self, jindx, iindx, dirName_b0, figname_b0, ip, indx_sea, indx_land, ndx_alldomain,
                       index_neares):
        dmet = self.dmet
        print("\n###########################\n"
        ...
        def Temp(air_temperature_2m=True, air_temperature_ml=True, air_temperature_0m=True, subplot1_labels=[],
            ...
            if dmet.air_temperature_ml is not None:
                TML0 = axm1.plot(dmet.time_normal, dmet.air_temperature_ml[:, -1, jindx, iindx] - 273.15, "--",
                                 color="brown")
            ...
            if dmet.air_temperature_0m is not None:
                TS = axm1.plot(dmet.time_normal, dmet.air_temperature_0m[:, -1, jindx, iindx] - 273.15, "-.",
                               color="brown")
                axm1.set_ylabel('Temp ($^\circ$C)', color="brown")
                axm1.tick_params(axis="y", colors="brown")
                min = np.min([dmet.air_temperature_ml[:, -1, jindx, iindx],
                              dmet.air_temperature_0m[:, -1, jindx, iindx]]) - 273.15
                max = np.max([dmet.air_temperature_ml[:, -1, jindx, iindx],
                              dmet.air_temperature_0m[:, -1, jindx, iindx]]) - 273.15
        ...
        def RH():
            ...
            if dmet.relative_humidity_2m is not None:
                axm1_2 = axm1.twinx()
                axm1_2.plot(dmet.time_normal, dmet.relative_humidity_2m[:, 0, jindx, iindx] * 100, color="seagreen",
                            alpha=0.8)
        ...
        def q(subplot2_labels=[], labeltext2=[]):
            ...
            if dmet.specific_humidity_2m is not None:
                Q2M = axm2.plot(dmet.time_normal, dmet.specific_humidity_2m[:, 0, jindx, iindx] * 1000, zorder=2,
                                color="green",
                                alpha=0.8)
            ...
            if dmet.specific_humidity_ml is not None:
                QML0 = axm2.plot(dmet.time_normal, dmet.specific_humidity_ml[:, -1, jindx, iindx] * 1000, "--",
                                 zorder=2,
                                 color="green",
                                 alpha=0.8)
        ...
        def CloudType(subplot2_labels=[], labeltext2=[]):
            ...
            if dmet.cloud_area_fraction is not None:
                tot_clf_f = axm2_1.fill_between(dmet.time_normal, 0,
            ...
            if dmet.high_type_cloud_area_fraction is not None:
                ...
                low_clf = axm2_1.plot(dmet.time_normal, dmet.low_type_cloud_area_fraction[:, -1, jindx, iindx] * 100,
                                      zorder=2,
        ...
        def pressure(subplot3_labels=[], labeltext3=[]):
            ...
            if pressure is not None:
                axm3_2 = axm3.twinx()
                P = axm3_2.plot(dmet.time_normal, dmet.surface_air_pressure[:, 0, jindx, iindx] / 100, zorder=1,
                                color="k")
        ...
        def precip_type(subplot4_labels=[], labeltext4=[]):
            ...
            if dmet.snowfall_amount is not None:
                ...
                rain_frac = \
                    np.divide(dmet.rainfall_amount[:, -1, jindx, iindx], tot,
                              out=np.zeros(np.shape(dmet.rainfall_amount[:, -1, jindx, iindx])), where=tot != 0.) * 100
                snow_frac = \
                    np.divide(dmet.snowfall_amount[:, -1, jindx, iindx], tot,
                              out=np.zeros(np.shape(dmet.snowfall_amount[:, -1, jindx, iindx])), where=tot != 0.) * 100
                graupel_frac = \
                    np.divide(dmet.graupelfall_amount[:, -1, jindx, iindx], tot,
                              out=np.zeros(np.shape(dmet.graupelfall_amount[:, -1, jindx, iindx])),
                              where=tot != 0.) * 100

                wd = -0.125

                p1 = axm4_1.bar(dmet.time_normal, rain_frac, width=wd / 4, alpha=0.3, color="red", zorder=0)
                p2 = axm4_1.bar(dmet.time_normal, snow_frac, width=wd / 4, bottom=rain_frac, alpha=0.3, color="aqua",
                                zorder=0)
                p3 = axm4_1.bar(dmet.time_normal, graupel_frac, width=wd / 4, bottom=rain_frac + snow_frac, alpha=0.3,
                                color="gray", zorder=0)
        ...
        def samples(subplot5_labels=[], labeltext5=[]):
            ...
            if dmet.samplesize is not None:
                ...
                S_bar = axm5.bar(dmet.time_normal, dmet.samplesize[:, jindx, iindx] if len(np.shape(dmet.samplesize))==3 else dmet.samplesize[:,0, jindx, iindx], ls="--",
        ...
        def plot_dexcess(subplot5_labels=[], labeltext5=[]):
            ...
            if dmet.dexcess is not None:
                ...
                S_dx = axm5_1.plot(dmet.time_normal, dmet.dexcess[:, jindx, iindx] if len(np.shape(dmet.dexcess))==3 else dmet.dexcess[:,0, jindx, iindx], color="blue")
        ...
        def loc2map(subplot6_labels=[], labeltext6=[]):
            ...
            axm6_0.set_xticks([]); axm6_0.set_yticks([]) #hide ticks
            axm6_0.set_facecolor("lightgray")

            #map plot
            sea_color =[97/255,126/255,156/255] #stock_img color default
            crs = default_map_projection(self.dmet)
            #zoomed in map
            import cartopy.io.img_tiles as cimgt
            stamen_terrain = cimgt.Stamen('terrain-background')

            axm6 = figm3.add_subplot(3, 3, 8, projection=crs,facecolor=sea_color)#, facecolor='lightskyblue') #axisbg='red'
            #axm6 = figm3.add_subplot(3, 3, 8, projection=stamen_terrain.crs,facecolor='lightskyblue')#, facecolor='lightskyblue') #axisbg='red'
            ...
            axm6.add_wms(wms='http://vmap0.tiles.osgeo.org/wms/vmap0',layers=['basic'])

            #axm6.add_image(stamen_terrain,10) #add, 8 for zoom level

            # axm6.add_feature(cfeature.GSHHSFeature(scale='high'), facecolor='whitesmoke', edgecolor='k', linewidths=2.,
            #                 zorder=0)

            mainpoint = axm6.scatter(self.point_lonlat[0], self.point_lonlat[1], c="blue", s=6 ** 2,
                                     transform=ccrs.PlateCarree(), zorder=2)
            allpoint = axm6.scatter(dmet.longitude, dmet.latitude, c="black", s=1 ** 2, transform=ccrs.PlateCarree(),
                                    zorder=1)
            evalpoint = axm6.scatter(dmet.longitude[jindx, iindx], dmet.latitude[jindx, iindx], c="red", s=6 ** 2,
                                     transform=ccrs.PlateCarree(), zorder=4)
            if len(dmet.longitude) <2: #if we dont have a domain with many points, make a extent
                axm6.set_extent([self.point_lonlat[0]-0.1, self.point_lonlat[0]+0.1, self.point_lonlat[1]-0.1, self.point_lonlat[1]+0.1], crs=ccrs.PlateCarree())
            #big map
            axm6_1 = figm3.add_subplot(3, 3, 9, projection=crs, facecolor= sea_color)
            #axm6_1.add_wms(wms='http://vmap0.tiles.osgeo.org/wms/vmap0',
            #             layers=['basic'])

            URL = 'http://gibs.earthdata.nasa.gov/wmts/epsg4326/best/wmts.cgi'
            from owslib.wmts import WebMapTileService
            wmts = WebMapTileService(URL)
            axm6_1.coastlines(facecolor='k', edgecolor='k', linewidths=1.)
            axm6_1.add_wmts(wmts,
                          layer_name='MODIS_Terra_CorrectedReflectance_Bands367', #MODIS_Terra_CorrectedReflectance_Bands367 MODIS_Terra_SurfaceReflectance_Bands143
                          wmts_kwargs={'time': f"{self.date[0:4]}-{self.date[4:6]}-{self.date[6:8]}"}, alpha = 0.7)

            #axm6_1.add_feature(cfeature.GSHHSFeature(scale='low'), facecolor='whitesmoke', edgecolor='k', linewidths=1.,
            #                   zorder=0)
            #axm6_1.coastlines(facecolor='k', edgecolor='k', linewidths=1.,
            #                   zorder=0)
            #axm6_1.add_feature(cfeature.LAND, facecolor='#AAAAAA')
            #axm6_1.add_feature(cfeature.COASTLINE, facecolor='k', zorder=0)

            axm6_1.scatter(dmet.x[iindx], dmet.y[jindx], facecolors='none', edgecolors='r', s=6 ** 2, zorder=1,
                           marker='s')
            axm6_1.set_extent([0, 25, 50, 85], crs=ccrs.PlateCarree())
            axm6_1.set_zorder(4)
            #make red zoomed in lines
            con = ConnectionPatch(xyA=(dmet.x[iindx], dmet.y[jindx]), xyB=(1, 1), coordsA="data",
                                  coordsB="axes fraction", axesA=axm6_1, axesB=axm6, color="red", linewidth=1)
            ...
            labeltext6 += [f"requested pos.\n[{self.point_lonlat[0].item()},{self.point_lonlat[1].item()} ]\n {self.point_name}",
                           "model gridpoints", "evaluated gridpoint"]
            axm6_L = figm3.add_subplot(3, 3, 7)
            axm6_L.set_xticks([]); axm6_L.set_yticks([]) #hide ticks
            axm6_L.axis("off")
        ...
        #takes forever...when pnly 1-2 time steps, and generally quite some time.
        figm3.savefig(dirName_b0 + figname_b0 + "_op2" + ".png", bbox_inches="tight", dpi=200)
    ...
    def meteogram_average(self, indx, dirName_b2, figname_b2, sitename):
        ...
        def Temp(air_temperature_2m=True, air_temperature_ml=True, air_temperature_0m=True, subplot1_labels=[],
            ...
            if dmet.air_temperature_2m is not None:
                ...
                T2M = axma1.plot(dmet.time_normal, dmet.air_temperature_2m[:, 0, indx[0], indx[1]] - 273.15,
                                 color="red",
                                 linewidth=0.2,
                                 alpha=0.7)
        ...
        def RH(subplot1_labels=[], labeltext=[]):
            ...
            if dmet.relative_humidity_2m is not None:
                ...
                RH2M = axma1_2.plot(dmet.time_normal, dmet.relative_humidity_2m[:, 0, indx[0], indx[1]] * 100,
                                    color="seagreen",
        ...
        def Precip(subplot1_labels=[], labeltext=[]):
            ...
            if dmet.precip1h is not None:
                ...
                P_bar_max = axma1_3.bar(dmet.time_normal, precip_max, color="lightblue", alpha=0.5, width=wd / 4,
                                        align="edge",
                                        bottom=0,
                                        zorder=10)
        ...
        def CloudType(subplot2_labels=[], labeltext2=[]):
            ...
            if dmet.low_type_cloud_area_fraction is not None:
                ...
                high_clf = axma2_1.plot(dmet.time_normal, all_high_clf * 100, zorder=2, color="lightblue",
                                        marker=r"$C_H$",
                                        markersize=12)
        ...
        def wind(subplot3_labels=[], labeltext3=[]):
            ...
            if dmet.y_wind_10m is not None:
                wspeed = np.sqrt(
                    dmet.x_wind_10m[:, 0, indx[0], indx[1]] ** 2 + dmet.y_wind_10m[:, 0, indx[0], indx[1]] ** 2)
            ...
            if dmet.y_wind_gust_10m is not None:
                wspeed_gust = np.sqrt(
                    dmet.x_wind_gust_10m[:, 0, indx[0], indx[1]] ** 2 + dmet.y_wind_gust_10m[:, 0, indx[0],
                                                                        indx[1]] ** 2)
        ...
        def fluxes(subplot4_labels=[], labeltext4=[]):
            ...
            if dmet.H is not None:
                ...
                P_LH = axma4.plot(dmet.time_normal, dmet.LE[:, indx[0], indx[1]], zorder=1, color="orange",
                                  linewidth=0.2,
                                  alpha=0.7)
        ...
        def loc2map(subplot6_labels=[], labeltext6=[]):
            ...
            allpoint = axma6.scatter(dmet.longitude, dmet.latitude, c="black", s=5 ** 2, transform=ccrs.PlateCarree(),
                                     zorder=1)
            evalpoint = axma6.scatter(dmet.longitude[indx[0], indx[1]], dmet.latitude[indx[0], indx[1]], c="red",
                                      s=6 ** 2,
                                      transform=ccrs.PlateCarree(), zorder=4)
            ...
            axma6_1.add_feature(cfeature.GSHHSFeature(scale='low'), facecolor='whitesmoke', edgecolor='k',
                                linewidths=1.,
                                zorder=0)
...
if __name__ == "__main__":
    ...
    dirName_b0, dirName_b1, dirName_b2, dirName_b3, figname_b0, figname_b1, figname_b2, figname_b3 = setup_met_directory(
            args.datetime, args.point_name, args.point_lonlat, args.model, runid=args.id, outpath=args.outpath)

    VM = PMET(date=args.datetime, steps=args.steps, model=args.model, domain_name=args.domain_name[0],
    ...
    print(points)
    print(indx_sea)
    print(indx_alldomain)
    print(index_neares)

    for po in points:
            jindx, iindx = po
            VM.plot_meteogram(jindx, iindx, dirName_b0, figname_b0, ip, indx_sea, indx_land, indx_alldomain,
                              index_neares)
            plt.close("all")
            ip += 1
