...
def plot_meteogram_vertical(dmet, dmet_sfx, dmet_ml, jindx, iindx, dirName_b1, figname_b1, ip):
    from weathervis.calculation import _pl2alt_full2full_gl
    ...
    p_top = 500
    z = _pl2alt_full2full_gl( dmet.air_temperature_ml, dmet.specific_humidity_ml,dmet_ml.p)
    ...
    areafrac_cloud = np.ma.array(dmet_ml.cloud_area_fraction_ml[:, :, jindx, iindx], mask=p < p_top)
    areafrac_cloud=areafrac_cloud*100
    rh_p = relative_humidity(air_tempC_p, dmet_ml.specific_humidity_ml[:, :, jindx, iindx], dmet_ml.p[:, :, jindx, iindx] )
    ...
    axm1.contour(tx, p_p, q_p, linestyles="dashed",
                 levels=ticks[1:-1], colors="white", linewidth=2, zorder=2, alpha=0.8)
    ...
    axm2.barbs(tx[skip][skip], p_p[skip][skip], uml_p[skip][skip] * 1.943844,
               vml_p[skip][skip] * 1.943844, length=7, zorder=1000, sizes=dict(emptybarb=0.25, spacing=0.15, height=0.4))
    ...
    CS = axm2.contour(tx, p_p, dmet_ml.theta[:, :, jindx, iindx], colors="black", levels=lvl,
                      zorder=2)
    ...
    #RH
    C = axm2.contour(tx, p, rh_p, zorder=2, levels = np.arange(0,100,10),colors="green") #dtdz_p
...
def plot_meteogram(dmet, dmet_sfx, dmet_ml, jindx, iindx, dirName_b0, figname_b0, ip):
    ...
    print(topidx)
    axm1_3.set_ylim(bottom=0.001, top=topidx+0.1*topidx) # adding 20% of max value for getting some clearense above bar
    ...
    tot_clf_f = axm2_1.fill_between(dmet.time_normal, 0, dmet.cloud_area_fraction[:, -1, jindx, iindx] * 100, zorder=1,color="gray", alpha=0.6)
    tot_clf = axm2_1.plot(dmet.time_normal, dmet.cloud_area_fraction[:, -1, jindx, iindx] * 100, zorder=1,color="k")
    tot_patch = mpl.patches.Patch(color='r', alpha=0.5, linewidth=0)

    conv_clf = axm2_1.plot(dmet.time_normal, dmet.convective_cloud_area_fraction[:, -1, jindx, iindx] * 100,zorder=2,color="pink")
    high_clf = axm2_1.plot(dmet.time_normal, dmet.high_type_cloud_area_fraction[:, -1, jindx, iindx] * 100,zorder=2,color="lightblue", marker=r"$C_H$", markersize=12)
    med_clf = axm2_1.plot(dmet.time_normal, dmet.medium_type_cloud_area_fraction[:, -1, jindx, iindx] * 100,zorder=2,color="blue", marker=r"$C_M$", markersize=12)
    low_clf = axm2_1.plot(dmet.time_normal, dmet.low_type_cloud_area_fraction[:, -1, jindx, iindx] * 100, zorder=2,color="red", marker=r"$C_L$", markersize=12)
    ...
    axm2_1.legend([Q2M[0], QML0[0], (tot_clf[0],tot_patch), high_clf[0],med_clf[0],low_clf[0],conv_clf[0]],
                ["2m Spec.Hum.", "ml0 Spec.Hum.", "tot.cloud", "hi. cloud", "med. cloud", "low cloud", "conv. cloud"],
                loc='upper left').set_zorder(99999)
    ...
    # label
    axm3_2.legend([GUST[0], WIND[0]], ["10m wind gust", "10m wind (10min mean)"], loc='upper left').set_zorder(99999)
    ...
    #rainfall_amount
    axm4_1 = axm4.twinx()
    tot = dmet.rainfall_amount[:, -1, jindx, iindx] + dmet.snowfall_amount[:, -1, jindx, iindx] + dmet.graupelfall_amount[:, -1, jindx, iindx]
    ...
    graupel_in = axm4_1.plot(dmet.time_normal, graupel_frac, zorder=1,color="lightblue",linestyle='dashed', marker='D')
...
def plot_maplocation(dmet, data_domain, close2point, dirName_b2, figname_b2, sitename="ALL", point_lonlat=None,
                     all=False):
    ...
    for p in close2point:
        ...
        all_gridpoint = ax.plot(dmet.longitude, dmet.latitude, marker='.', markersize=6.0, markeredgewidth=4,
                                transform=ccrs.PlateCarree(),
                                markerfacecolor='red', markeredgecolor='black', zorder=1000, linestyle='None')

        # print(point_lonlat[0])
        # print(point_lonlat[1])
        mainpoint = ax.plot(point_lonlat[0], point_lonlat[1], marker='.', markersize=5.0, markeredgewidth=4,
                            transform=ccrs.PlateCarree(),
                            markerfacecolor='blue', markeredgecolor='blue', zorder=1000, linestyle='None')

        ax.plot(dmet.longitude[p], dmet.latitude[p], marker='.', markersize=6.0, markeredgewidth=4,
                transform=ccrs.PlateCarree(),
                markerfacecolor='red', markeredgecolor='red', zorder=1000, linestyle='None')

        CC = plt.contour(dmet.longitude, dmet.latitude, dmet.land_area_fraction[0, 0, :, :], alpha=0.6, zorder=3,
                         levels=[0.9, 1, 1.1],
...
def meteogram_average(dmet, dmet_sfx, dmet_ml, indx, dirName_b2, figname_b2, sitename):
    ...
    high_clf = axma2_1.plot(dmet.time_normal, all_high_clf * 100,zorder=2,color="lightblue", marker=r"$C_H$", markersize=12)
    med_clf = axma2_1.plot(dmet.time_normal, all_mid_clf * 100,zorder=2,color="blue", marker=r"$C_M$", markersize=12)
    low_clf = axma2_1.plot(dmet.time_normal, all_low_clf * 100, zorder=2,color="red", marker=r"$C_L$", markersize=12)
    ...
    axma3_3.legend([GUST_MEAN[0], WIND_MEAN[0],PP[0]], ["10m wind gust", "10m wind (10min mean)", "mean surf. pressure"], loc='upper left').set_zorder(
        99999)
    ...
    P_SH = axma4.plot(dmet.time_normal, dmet_sfx.H[:, indx[0], indx[1]], zorder=0, color="blue", linewidth=0.2,
                      alpha=0.7)
    P_LH_MEAN = axma4.plot(dmet.time_normal, LH_mean, zorder=0, color="orange", linewidth=3, alpha=1)
    P_LH = axma4.plot(dmet.time_normal, dmet_sfx.LE[:, indx[0], indx[1]], zorder=0, color="orange", linewidth=0.2,
                      alpha=0.7)
...
def read_data(dt, steps, model, domain_name, domain_lonlat):
    ...
    param_SFC = ["air_temperature_2m", "air_temperature_0m", "surface_air_pressure", "air_pressure_at_sea_level",
                 ...
                 "precipitation_amount_acc", "land_area_fraction", "cloud_base_altitude","cloud_top_altitude",
                 "convective_cloud_area_fraction","cloud_area_fraction","high_type_cloud_area_fraction",
                 "medium_type_cloud_area_fraction","low_type_cloud_area_fraction","rainfall_amount","snowfall_amount",
                 "graupelfall_amount"]
...
def meteogram(datetime, steps=[0, 10], model="AromeArctic", domain_name="KingsBay_Z1", point_name=None,
              point_lonlat=None, num_point=1, domain_lonlat=None, plot="all", legend=True, info=False,
              save=True):  ##python T850_RH.py --datetime 2020091000 --steps 0 1 --model MEPS --domain_name West_Norway
    for dt in datetime:
        ...
        for sitename in averagesite:
            ...
            if sitename == "ALL_NEAREST":
                ...
                plot_maplocation(dmet, data_domain, [jindx, iindx], dirName_b2, figname_b2, sitename, point_lonlat,
                                 all=True)
            if sitename == "ALL_DOMAIN":
                ...
                plot_maplocation(dmet, data_domain, indx_alldomain, dirName_b2, figname_b2, sitename, point_lonlat,
                                 all=True)
