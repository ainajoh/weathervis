...
from weathervis.utils import filter_values_over_mountain, default_map_projection, mslp_contour, plot_by_subdomains
...
def plot_OLR(datetime,dmet,figax=None, scale=1, lonlat=None, steps=[0,2], coast_details="auto", model=None, domain_name=None,
  ...
  for leadtime in np.array(steps): #
      ...
      ax1 = mslp_contour(dmet.x, dmet.y, MSLP[itim, 0, :, :], ax1, scale=scale)
      ...
      data[mask] = np.nan
      ax1.pcolormesh(x, y, data[ :, :],cmap=plt.cm.Greys_r)#vmin=-230,vmax=-110, cmap=plt.cm.Greys_r)
